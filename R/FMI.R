...
DailyWeather <- setRefClass(
  ...
  methods = list(
    ...
    interpolateField = function(field, zeroPositive) {
    ...
    interpolate = function(templateRaster=raster::raster(extent(85000, 726000, 6629000, 7777000), nrows=1148, ncols=641, crs=sp::CRS("+init=epsg:3067"))) {
      ...
      rasters <- raster::addLayer(raster::stack(),
                                  interpolateField("rrday", zeroPositive=T),
                                  interpolateField("tday", zeroPositive=F),
                                  interpolateField("snow", zeroPositive=T),
                                  interpolateField("tmin", zeroPositive=F),
                                  interpolateField("tmax", zeroPositive=F))
      return(rasters)
    }
  )
)
